(function () {
    ...
    function markSingleMedia() {
        document.querySelectorAll('article[data-testid="tweet"]').forEach((tweet) => {
            ...
            const mediaBox = photos[0].parentElement?.parentElement;
            if (!mediaBox || mediaBox === tweet) return;
            mediaBox.classList.remove("x2md-single-photo", "x2md-single-video");
            mediaBox.classList.add(tweet.querySelector('[data-testid="videoPlayer"], video')
                ? "x2md-single-video"
                : "x2md-single-photo");
        });
    }
